...
class LocalizationNotifier extends StateNotifier<LocalizationState> {
  LocalizationNotifier() : super(LocalizationState(locale: Locale("en "))) {
    _loadLocale();
  }

  Future<void> _loadLocale() async {
    String code = locator<SharedPreferencesHelper>().getString(AppStrings.kLocale) ?? "en";
  ...
  changeLanguage(String code) {
    state = state.copyWith(locale: Locale(code));
    locator<SharedPreferencesHelper>().setString(AppStrings.kLocale, code);
  }
}

final localizationProvider =
    StateNotifierProvider<LocalizationNotifier, LocalizationState>((ref) => LocalizationNotifier());
